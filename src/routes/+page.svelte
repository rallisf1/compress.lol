<script lang="ts">
	...
	const loadFFmpeg = async (): Promise<void> => {
		try {
			...
			await ffmpeg.load({
				coreURL: await toBlobURL(`https://ffmpeg.dws.gr/cdn/ffmpeg-core.min.js`, 'text/javascript'),
				wasmURL: await toBlobURL(`https://ffmpeg.dws.gr/cdn/ffmpeg-core.wasm`, 'application/wasm'),
